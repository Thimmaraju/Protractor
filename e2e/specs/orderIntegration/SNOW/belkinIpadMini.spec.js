...
var logGenerator 	= 		require("../../../../helpers/logGenerator.js"),
	...
	belkinIpadMiniCaseTemplate = require('../../../../testData/OrderIntegration/SNOW/BelkinIPadMiniCase.json');

describe('SNOW : Belkin iPad Mini Case Template', function () {
    ...
    var messageStrings = {
            ...
            estimatedPrice : 'USD 50.00 ONE TIME CHARGE + USD 0.00 / MONTH',
    ...
    it('SNOW : Belkin Ipad Mini Case ---- Verify fields on Main Parameters page of Development Laptop is working fine', function () {
        catalogPage.clickConfigureButtonBasedOnName(belkinIpadMiniCaseTemplate.bluePrintName);
        expect(util.getCurrentURL()).toMatch(appUrls.placeOrderMainParamtersPageUrl);
        var belkinIpadMiniObject = JSON.parse(JSON.stringify(belkinIpadMiniCaseTemplate));
        expect(placeOrderPage.getTextBluePrintName()).toContain(belkinIpadMiniCaseTemplate.descriptiveText);
        placeOrderPage.setServiceNameText("QAAUTOMATION" + util.getRandomString(4));
        expect(placeOrderPage.isNextButtonEnabled()).toBe(true);
        //expect(placeOrderPage.getTextProvider()).toBe(messageStrings.providerName);
        //expect(placeOrderPage.getTextCategory()).toBe(messageStrings.category);
        expect(placeOrderPage.getTextEstimatedPrice()).toBe(belkinIpadMiniCaseTemplate.EstimatedPrice);
    });
    
    it('SNOW : Belkin Ipad Mini Case ---- Verify Summary details and Additional Details are listed in review Order page', function () {
        var belkinIpadMiniObject = JSON.parse(JSON.stringify(belkinIpadMiniCaseTemplate));
        catalogPage.clickConfigureButtonBasedOnName(belkinIpadMiniCaseTemplate.bluePrintName);
        orderFlowUtil.fillOrderDetails(belkinIpadMiniCaseTemplate, modifiedParamMap).then(function (requiredReturnMap) {
    ...
    it('SNOW : Belkin Ipad Mini Case ---- Verify Order is listed in Orders details page once it is submitted from catalog page', function () {
        var belkinIpadMiniObject = JSON.parse(JSON.stringify(belkinIpadMiniCaseTemplate));
        catalogPage.clickConfigureButtonBasedOnName(belkinIpadMiniCaseTemplate.bluePrintName);
        orderFlowUtil.fillOrderDetails(belkinIpadMiniCaseTemplate, modifiedParamMap);
        ...
        //expect(ordersPage.getTextTeamOrderDetails()).toEqual(jsonUtil.getValue(belkinIpadMiniObject, "Team"));
      ...
      //expect(ordersPage.getTextSubmittedByOrderDetails()).toEqual(browser.params.username);
        ...
        expect(ordersPage.getTextBasedOnLabelName("Quantity")).toEqual(jsonUtil.getValue(belkinIpadMiniObject, "Quantity"));
        ordersPage.clickBillOfMaterialsTabOrderDetails();
        //expect(ordersPage.getEstimatedCostFromEstimatedCostTab()).toEqual('USD50.00');
    });
	
    if(isProvisioningRequired == "false") {
        it("SNOW : Belkin Ipad Mini Case ---- Verify provisioning of Belkin Ipad service is working fine from consume App", function(){
            var orderObject = {};
            var belkinIpadMiniObject = JSON.parse(JSON.stringify(belkinIpadMiniCaseTemplate));
            catalogPage.clickConfigureButtonBasedOnName(belkinIpadMiniCaseTemplate.bluePrintName);
